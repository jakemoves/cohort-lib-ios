...
@implementation CHSoundSampleCue
...
- (id)initWithSession: (CHSession *)session andAsset:(CHSoundAsset *)asset withTriggers:(NSArray *)triggers withTags:(NSSet *)tags error:(NSError **)error withCompletionBlock:(CHVoidBlock)completionBlock {
    if (self = [super init]) {
        ...
        if(!_session || !_asset){
            ...
            _audio = [AEAudioFilePlayer audioFilePlayerWithURL:_asset.sourceFile audioController:_session.audioController error:nil];
            _audio.channelIsPlaying = NO;
            _audio.loop = false;
            _audio.volume = 1.0;
        }
    }
